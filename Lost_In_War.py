...
pygame.display.set_caption('Lost_In_War By JediCubing')
...
ROWS = 16
COLS = 150
TILE_SIZE = SCREEN_HEIGHT // ROWS  # = 50
TILE_TYPES = 21    # How many types of tIles are there?
screen_scroll = 0
bg_scroll = 0
level = 1
start_game = False
		...
		for tile in world.obstacle_list:
			...
			if tile[1].colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
				...
				if self.char_type == "enemies":
					self.direction *= -1
					self.move_counter = 0
			#check for collision in the y derection
			if tile[1].colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
				#Check if below ground
				if self.vel_y < 0:
					self.vel_y = 0
					dy = tile[1].bottom - self.rect.top
				#Check if above or falling
				elif self.vel_y >= 0:
					self.vel_y = 0
					self.in_air = False
		...
		if self.alive and player.alive:
			...
			if self.vision.colliderect(player.rect):
				...
				if self.idling == False:
					...
					self.move_counter += 1
					#update ai vision as the enemies moves
			...
			for x, tile in enumerate(row):
				if tile >= 0:
					...
					if tile >= 0 and tile <= 8:
						...
						player = Soldier('player', x * TILE_SIZE, y * TILE_SIZE, 1.65, 5, 16, 5)
						health_bar = HealthBar(10, 10, player.health, player.health)
					elif tile == 16:#create enemies
						enemies = Soldier('enemies', x * TILE_SIZE, y * TILE_SIZE, 1.65, 2, 20, 0)
...
class ItemBox(pygame.sprite.Sprite):
	...
	def update(self):
		...
		if pygame.sprite.collide_rect(self, player):
			...
			elif self.item_type == 'Ammo' and player.ammo < 16:
				player.ammo += 5
...
death_fade = ScreenFade(2, Colour, 4)
...
while run:
	...
	if start_game == False:
		...
		#Instructions
		draw_text('INSTRUCTIONS: USE ARROWS TO MOVE', font, MIDNIGHT_BLUE, 250, 20)
		draw_text('PRESS "W" TO LAUNCH A GRENADE (50 DAMAGE)', font, MIDNIGHT_BLUE, 250, 40)
		draw_text('PRESS "SPACE" TO SHOOT A BULLET (25 DAMAGE)', font, MIDNIGHT_BLUE, 250, 60)
		...
		if player.alive:
			...
			if level_complete and MAX_LEVELS > level:
				start_intro = True
				level += 1
				bg_scroll = 0
